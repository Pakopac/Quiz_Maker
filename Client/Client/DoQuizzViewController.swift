...
class DoQuizzViewController: UIViewController {
    ...
    func enterScoreInDB(score: Int){
        let json: [String: Any] = [
          "quizId": self.id_quizz,
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let barVC = segue.destination as? UITabBarController {
            barVC.viewControllers?.forEach {
                if let vc = $0 as? MyScoreViewController {
                    vc.score = score
                }
                else if let vc = $0 as? AllScoresTableViewController{
                    vc.quizz_id = id_quizz
                }
            }
        }
     }
