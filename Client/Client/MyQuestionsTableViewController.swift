...
class MyQuestionsTableViewController: UITableViewController {
          ...
          request.httpMethod = "GET"
           ...
           let task = URLSession.shared.dataTask(with: request) { data, response, error in
              do {
                ...
                self.jsonFilter = self.json.filter{ $0["author"] as! String == self.defaults.string(forKey: "name")}
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        print(jsonFilter.count)
        return jsonFilter.count
    }

    
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "MyQuestionsCell", for: indexPath)
